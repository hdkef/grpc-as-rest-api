...
func TestCreateAuth(t *testing.T) {
	...
	mockAuthRepoSuccess.On("CreateAuth", fakeEntity).Return(nil)
	mockAuthRepoFail := new(mocks.AuthRepository)
	mockAuthRepoFail.On("CreateAuth", fakeEntity).Return(errors.New("some error"))
	...
	for _, v := range testcases {
		t.Run(v.Name, func(t *testing.T) {
			...
			//execute
			err := authUsecase.CreateAuth(v.Args.auth)
